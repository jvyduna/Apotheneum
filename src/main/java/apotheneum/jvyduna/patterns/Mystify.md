...
- **Wrap drawing**: on wrapping topologies each edge takes the short way around
  ...
  the flash cross's ±1 x-neighbors are explicitly edge-guarded — `set()`'s x
  wrap would otherwise smear a face-edge flash onto the opposite edge.
- **Zero-alloc render**: canvases, position/velocity/sync-scale arrays, and the
  4-entry color array are constructed once; regeneration happens only in trigger
  handlers and event-rate sync re-plans. Per-frame work is ~10 vertex
  integrations, ≤10 lines, one decay pass.
...
attached via `AudioReactive.setDepth(...)`:

- **Knob at 0 (default)**: every tap reads its silence value — this *is* the
  baseline look. Speed sits steady at 0.7× nominal (the low end of the level
  breath: graceful, unvarying drift), no bass flashes ever fire, trails stay at
  their full knob-set length. A complete ambient screensaver.
- **Raising the knob** continuously restores, scaled by depth:
  - `level` (smoothed 0..1) — breathes speed ±30% around a nominal that is
    ...
    At partial depth the breath range shrinks proportionally (level scales
    linearly with depth).
  - `bassHit()` — fires once depth > 0.01; arms the flash envelope to
    `audio.depth()` (not fixed 1), so the white vertex-cross pop brightness
    scales with the knob. The cross (vertex pixel + 4 edge-guarded neighbors)
    fades linearly over `FLASH_DECAY_MS = 500` ms; hits can be gated as close
    as ~an eighth note, which simply re-arms the envelope. Stamped onto the
    canvas, so the pop also leaves a decaying trail residue.
  - `trebleRatio` — subtly shortens trails: half-life divided by
    ...
    full depth.
- Knob at 1 is bit-identical to the pre-knob fully-reactive behavior.

## Tempo mapping

Motion points locked to the grid when `Sync` (default on) is enabled, against
`TempoDiv` (default QUARTER):

- **Vertex wall bounces** are the pattern's only motion events:
  - **y bounces** off floor (y=1) and ceiling (y=0) in *all* geometry modes;
  - **x bounces** off left/right face edges in FACES mode only (x wraps in
    CUBE_RING/CYLINDER — a wrap is not an event, so x sync scale stays 1 there).
- **Mechanism**: velocities are never mutated by sync. Each vertex axis carries
  a sync scale (`syncScaleX/Y`, preallocated); effective velocity =
  base velocity × scale. At each event-scheduling moment the time to that
  axis's next wall hit is computed at the *base* rate and
  `TempoLock.retime(msUntil, div, 0.7, min(1.4, 1/|vel|))` **replaces** the
  scale — scales never compound, so no random-walk drift.
- **Re-plan cadence** (all event-rate, zero per-frame allocation): each bounce
  re-plans its own vertex axis; a full re-plan of all 10 vertices runs on sync
  engage, TempoDiv change, geometry change, Scatter/Reverse triggers, and on
  every TempoDiv crossing (`TempoLock.crossed`) — the crossing cadence cheaply
  absorbs live BPM changes and Energy/Speed knob moves mid-flight.
- **Clamp override rationale**: the upper clamp is `min(1.4, 1/|vel|)` instead
  of the default 1.4. Velocity magnitudes are in cap units (≤ 1), so
  |vel|·scale ≤ 1 always — retiming can *never* push a vertex past the
  energy-set traversal cap (see compliance math below). Fast vertices
  (|vel| → 1) can only be retimed by slowing; slow vertices keep the full
  [0.7, 1.4] window. Lower clamp is the 0.7 default.
- **Audio interaction**: planning uses the instantaneous rate. With Audio at 0
  (default) the rate is steady, so bounces land precisely on the grid. With
  Audio raised, the ±30% level breath deliberately wins over exact landings;
  the per-crossing re-plan keeps bounces near-grid. CURATE: verify bounces
  read as on-beat with Audio at ~0.5 on real music.
- **Sync off**: scales reset to 1 on the toggle frame; motion is bit-identical
  to the pre-sync free-running pattern (velocities were never touched).
- CURATE: per-crossing re-plans can re-target a bounce to an adjacent boundary
  mid-flight (a bounded ≤±few-% speed nudge far from walls, larger only very
  near a wall where it's pixel-invisible); confirm no perceptible lurch.
- CURATE: with QUARTER default and ~10–18 s per span, each vertex bounces
  rarely, but 10 vertices × 2 axes give a steady trickle of on-beat wall hits;
  verify the beat-lock is legible, else try EIGHTH or HALF as default.
...
UI/registration order: triggers, Energy, pattern parameters, Audio, Sync,
TempoDiv, Meta last.
...
| `sync` | Sync | BooleanParameter | true | — | lock vertex wall-bounces to the tempo grid |
| `tempoDiv` | TempoDiv | EnumParameter&lt;Tempo.Division&gt; | QUARTER | all divisions | grid that bounces land on when Sync is on |
...
- `scatter` — every vertex jumps to a fresh random position with a fresh random
  velocity (and sync scales re-plan). The old shapes' trails fade out at the
  trail rate (~0.3–2 s at typical knob settings) while the new shapes draw in,
  so the cut reads within a second or two without a hard blackout. (Largest.)
- `reverse` — all velocities negate; the shapes visibly fold back along their
  own fading trails. Reads immediately because each line re-enters its own
  ghost. (Mid-size.)
...
**Sync interaction.** Retiming multiplies a vertex axis by a scale clamped to
`[0.7, min(1.4, 1/|vel|)]`. Since |vel| ≤ 1 in cap units,
`|vel| · scale ≤ |vel| · 1/|vel| = 1` — the effective per-axis speed still
never exceeds `rate ≤ cap`, so the table above and the ≥5 s floor hold with
Sync on or off. Slow side: the slowest synced component is
`0.45 · 0.7 ≈ 0.32` of cap — CURATE: confirm this never reads as a stuck
vertex.

Fastest possible full-canvas-width traversal:

- **Default energy (0.35)**: cap = 1 / lin(0.35, 12, 5) = 1/9.55 → **9.55 s**
  minimum (loud audio at Audio=1, Speed = 1). Nominal (Audio knob at 0):
  9.55 × 1.3/0.7 ≈ 17.7 s.
- **Energy = 1**: cap = **5.0 s** minimum — exactly the ≥5 s floor, only
  reachable with Speed = 1 *and* Audio = 1 *and* sustained full level (a
  1.4× retime cannot break it, per the clamp above).
...
- `VEL_MIN = 0.45` — spread between slowest and fastest vertex; too low may look
  like a stuck vertex (and sync's 0.7 floor can dip a component to ~0.32 of cap).
...
- Audio-knob-0 speed of 0.7× nominal (the silence floor of the ±30% breath) —
  confirm ambient pacing still feels alive as the *default* look, since the
  knob now ships at 0.
...
- `TempoDiv = QUARTER` default and the per-crossing re-plan cadence — see the
  CURATE notes in the Tempo mapping section.
