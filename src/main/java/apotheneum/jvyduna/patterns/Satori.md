...
- **Field variants** (`Field` enum, rebuilt only on trigger/enum change, one
  ...
  - `ANGULAR` — mirrored azimuth sweep: triangle wave in x (seam-free at the
    ring wrap) × 1–3 arms, plus a signed vertical pitch term → helical
    chevrons around the sculpture (CURATE: pitch range ±0.5–1.5 cycles of
    tilt).
...
- `level` (extra-smoothed with a 1.2s time constant on top of AudioReactive's
  smoothing) modulates cycle speed ±50%: silence → 0.5×, average → 1×, loud →
  1.5×. Faster-than-base only when music is present; the cap math below
  budgets for the full 1.5×.
- `bassHit()` sets a pulse envelope to 1; the envelope decays linearly over
  2s. While active, each point's LUT phase is offset by
...
- `treble` lerps the whole LUT toward white by up to 30% — but only in the
  ...
  LUT pass, not per point.
- **Silence behavior**: with all taps at 0 the pattern is a steady palette
  rotation at 0.5× the energy-set rate — the core look, fully presentable.
  No hits fire, no shimmer, no pulse.
...
| Bass pulse depth (LUT cycles) | 0.15 | 0.35 | lin (`Ranges.lin`) |
...
- `newField` — the whole field snaps to a new random layout (new centers,
  wavelengths, arms, folds) in the current variant. Reads instantly as a scene
  change; the ongoing color rotation resumes over the new geometry, so it
  settles within one perceptual beat.
- `reverse` — band motion direction flips. Takes a few seconds to read at
  ambient rates (bands visibly change travel direction); instantaneous state
  change, no discontinuity in the image itself.

## Jump candidates

Rows mirror the `bag.jumpable(...)` lines in the constructor 1:1. Status is
updated during curation. (`newField` and `reverse` are also registered into
the bag as triggers.)
...
*entire* sculpture in `spread × cyclePeriod / audioBoost` seconds. Worst case
is the fastest configuration: `spread = 1`, `speed = 1`, max audio boost 1.5×:

- Cycle period `T(e) = Ranges.exp(e, 16, 8) = 16·(0.5)^e`
- **Default energy (0.35)**: `T = 16·2^(−0.35) ≈ 12.55s` → worst-case
  traversal `12.55 / 1.5 ≈ 8.4s ≥ 5s` ✓
- **Energy = 1**: `T = 8s` → worst-case traversal `8 / 1.5 ≈ 5.33s ≥ 5s` ✓
- At the actual defaults (`spread = 2`, no audio): traversal `2 × 12.55 ≈
  25s`; ambient (e=0) drift period is ≥ `16/1.5 ≈ 10.7s` even under max audio
  boost, satisfying the ≥8s ambient-drift requirement.
- The `speed` trim only divides *below* 1× rate, so it can never violate the
  cap; the meta jump subrange (0.4–1.0) respects this too.

**Event motion.** The bass pulse is event-like: an instantaneous radial warp
relaxing over 2.0s — above the 1.5s minimum visual life. It offsets phase by
at most 0.35 cycles, so it reads as a wave through the existing bands, not a
strobe.
